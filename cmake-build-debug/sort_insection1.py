...
with open('O2_sort_insertion.txt') as file:
    data = file.read().split('\n')
data = [x.split('; ') for x in data if x.find('; ') != -1]
file.close()
y2= [float(item[0]) for item in data]
...
with open('O3_sort_insertion.txt') as file:
    data = file.read().split('\n')
data = [x.split('; ') for x in data if x.find('; ') != -1]
file.close()
y2= [float(item[0]) for item in data]
...
plt.grid()
plt.legend((line5, line4, line3, line2, line1), ['sort_insertion_O3','sort_insection_O2', 'sort_insection_O1', 'sort_insection_O0', 'sort_insection'])
plt.show()
